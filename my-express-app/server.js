...
// MongoDB connection (redundant with connectDB, kept for logging)
...
app.put('/api/user/follow', authMiddleware, async (req, res, next) => {
  ...
  console.log('PUT /api/user/follow - Body:', req.body, 'User ID:', req.user.id); // Debug
  try {
    const user = await User.findById(req.user.id);
    if (!user) {
      console.log('User not found for ID:', req.user.id); // Debug
      return res.status(404).json({ message: 'User not found' });
    }
    console.log('Current following:', user.following); // Debug
    user.following = user.following.includes(community)
      ? user.following.filter(c => c !== community)
      : [...user.following, community];
    await user.save();
    console.log('Updated following:', user.following); // Debug
    res.json(user);
  } catch (error) {
    console.error('Follow error:', error); // Debug
    next(error);
  }
});
